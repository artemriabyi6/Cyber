...
// Додайте цей інтерфейс перед функцією компонента
...
export default function Dashboard() {
 ...
 const fetchDashboardData = async () => {
  try {
    ...
    console.log('Отримані дані тренувань:', trainingPlansData);

    // Трансформуємо дані для сумісності з існуючим інтерфейсом
    const transformedTrainingPlans: TrainingPlan[] = trainingPlansData.map((plan: ApiTrainingPlan) => ({
      id: plan.id,
      title: plan.title || `Тренування ${plan.date}`,
      duration: plan.duration,
      intensity: plan.intensity || "medium",
      completed: plan.completed,
      exercises: plan.exercises || [], // ВИПРАВЛЕННЯ: використовуємо exercises з плану
