...
export default function CoachDashboard() {
  ...
  const markTrainingCompleted = async (trainingId: string, userId: string) => {
    try {
      ...
      const sessionResponse = await fetch('http://localhost:3001/trainingSessions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const trainingResponse = await fetch(`http://localhost:3001/trainingPlans/${trainingId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!trainingResponse.ok) {
        throw new Error(`Помилка оновлення тренування: ${trainingResponse.status}`);
      }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <button className="w-full text-left p-4 bg-white border rounded-lg hover:border-green-500 hover:bg-green-50 transition-colors">
                      <div className="font-medium text-gray-900">📊 Переглянути статистику</div>
                      <div className="text-sm text-gray-600">Аналіз прогресу учнів</div>
                    </button>
